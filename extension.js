    ...
    getAsElement(title, data) {
        ...
        <div style="display: flex; flex-direction: column; gap: 2px;">
            ...
            <div style="display: flex; gap: 6px; align-items: center;">
                ${triangle}
                <span class="font-bold" style="font-size: 1.25rem">${Number(Math.round(data+'e2')+'e-2')}</span>
            </div>
        </div>`;
    }
}
...
(async () => {
    let buurt = document.getElementsByClassName("fd-m-left-2xs--bp-m fd-display-block fd-display-inline--bp-m")[0].innerText;
    ...
    container.innerHTML += data.getAsElement("Fysieke omgeving", data.environment);
    container.innerHTML += data.getAsElement("Overlast", data.nuisance);
    container.innerHTML += data.getAsElement("Sociale samenhang", data.socialCohesion);
    container.innerHTML += data.getAsElement("Voorzieningen", data.services);
    container.innerHTML += data.getAsElement("Woningvoorraad", data.housingSupply);
